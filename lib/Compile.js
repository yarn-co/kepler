...
var Compile = module.exports = function (options) {
      ...
      function createSVG(width) {
            ...
            // For use if stickerSheet is truthy
            
            /*
              There are 11 layers.
              The trixel (full hexagon) width for the stickerSheet
              is hard-coded at 36.6% of 9" page size --> 3.294"
              
              Bleed is expected to be 1/16" --> 0.0625" around the edges
              
              A layer crosses the diameter twice
              so each layer gets counted twice in the width
              
              3.294" (trixelWidth) / layersCount * 2 ==
              3.294" (trixelWidth) / 22 == 0.14972727" == layerWidth
              
              The bleed is 41.742563% of layerWidth
            */
            
            // Also gets used below when iterating through Geometry.all
            var bleedScale;
            var layerWidth;
            var bleedSize;
            
            if(options.stickerSheet) {
              bleedScale = 1.41742563;
              layerWidth = width / (layersCount * 2);
              bleedSize = layerWidth * bleedScale - layerWidth;
              var addedSizeFromBleed = layerWidth * bleedScale * 2;
              containerBoxWidth += addedSizeFromBleed;
              containerBoxHeight += addedSizeFromBleed;
            }
            ...
            Geometry.all.forEach(function(obj, key) {
              ...
              if(options.stickerSheet) {
                function addbleedSizeToVertex(v) {
                  if(v.x < 0) {
                    v.x -= bleedSize;
                  } else if(v.x > 0) {
                    v.x += bleedSize;
                ...
                if(key > 599 && key <= 659) {
                  // These are the trixel vertices on the outer ring pointing away from the center
                  //jsonData[key].color = "rgb(255,0,0)";
                  addbleedSizeToVertex(obj.v1);

                } else if(key > 659) {
                  // These are the trixel vertices on the outer ring pointing towards the center
                  //jsonData[key].color = "rgb(0,0,255)";
                  addbleedSizeToVertex(obj.v0);
                  addbleedSizeToVertex(obj.v1);
                  
                  // Trixels on the top and bottom apexes need special attention
                  if(key == 660) {
                    // Top trixel, left side
                    obj.v0.x = 0;
                    obj.v0.y += bleedSize/2;
                  }
                  if(key == 725) {
                    // Top trixel, right side
                    obj.v1.x = 0;
                    obj.v1.y += bleedSize/2;
                  }

                  if(key == 692) {
                    // Bottom trixel, left side
                    obj.v1.x = 0;
                    obj.v1.y -= bleedSize/2;
                  }
                  if(key == 693) {
                    // Bottom trixel, right side
                    obj.v0.x = 0;
                    obj.v0.y -= bleedSize/2;
                  }
                }
